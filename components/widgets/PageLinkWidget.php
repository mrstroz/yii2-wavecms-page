...
class PageLinkWidget extends Widget
{
    ...
    {
        if (!$this->form)
            throw new InvalidConfigException(Yii::t('wavecms_page/main', 'Attribute {attribute} is not defined', ['attribute' => 'form']));

        if (!$this->model)
            throw new InvalidConfigException(Yii::t('wavecms_page/main', 'Attribute {attribute} is not defined', ['attribute' => 'model']));
        ...
        $this->pages[Yii::t('wavecms_page/main', 'Pages')] = ArrayHelper::map($pages, 'id', 'title');
